...
class ProjectManager:
    ...
    def create_project(self, name):
        ...
        project = {
            ...
            'settings': {
                'bg_music_volume': 0.3  # 背景音乐默认音量(0.0-1.0)
            }
        }
    ...
    def load_project(self, project_id):
        ...
        if os.path.exists(project_file):
            with open(project_file, 'r', encoding='utf-8') as f:
                ...
                if 'background_music' not in project['files']:
                    project['files']['background_music'] = []
                # 确保项目包含设置
                if 'settings' not in project:
                    project['settings'] = {}
                if 'bg_music_volume' not in project['settings']:
                    project['settings']['bg_music_volume'] = 0.3
    ...
    def add_file(self, file_type, file_path):
        ...
        # 检查文件类型是否支持
        if file_type not in ['audio', 'images', 'background_music']:
    ...
    def update_setting(self, setting_name, value):
        ...
        if 'settings' not in self.current_project:
            self.current_project['settings'] = {}
            
        self.current_project['settings'][setting_name] = value
        self._save_project()
        return True
        
    def get_setting(self, setting_name, default=None):
        """获取项目设置"""
        if not self.current_project or 'settings' not in self.current_project:
            return default
            
        return self.current_project['settings'].get(setting_name, default)
